...
file_path = os.path.join(cmssw,"src/CMGTools/Production/crab/heppy_crab_config.py")
file_ = open(file_path, 'r' )
cfg = imp.load_source( 'cfg', file_path, file_)
config = cfg.config
...
config.General.workArea = 'crab_' + os.environ["DATETIME"] + '/crab_' + os.environ["DATASET"] + "_" + os.environ["LABEL"] # crab dir name
...
#final output: /store/user/$USER/output_dir/cmg_version/label/dataset/$date_$time/0000/foo.bar
config.Data.outLFNDirBase += '/' + os.environ["STAGEOUTREMDIR"] + '/' + os.environ["CMG_VERSION"]
config.Data.outputPrimaryDataset =  os.environ["LABEL"]
