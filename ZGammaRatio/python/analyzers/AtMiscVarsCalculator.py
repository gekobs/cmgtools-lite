...
class AtMiscVarsCalculator( Analyzer ):
    def __init__(self, cfg_ana, cfg_comp, looperName ):
        super(AtMiscVarsCalculator,self).__init__(cfg_ana,cfg_comp,looperName) 
    ...
    def declareHandles(self):
        super(AtMiscVarsCalculator, self).declareHandles()
       #genJets                                                                                                                                                                     
        self.handles['genJets'] = AutoHandle( 'slimmedGenJets','std::vector<reco::GenJet>')

    def beginLoop(self,setup):
        super(AtMiscVarsCalculator,self).beginLoop(setup)
    ...
    # print 'MET=',event.met.pt() 
        ...
        if len(event.selectedLeptons)>0:
            ...
            if self.cfg_ana.doMetNoHF:
                event.mtwNoHF = mtw(event.selectedLeptons[0], event.metNoHF)
                event.mtwDef = mtw(event.selectedLeptons[0], event.metDef)
